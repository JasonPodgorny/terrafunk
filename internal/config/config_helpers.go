...
func runCommand(ctx *ParsingContext, args []string) (string, error) {
	...
	if ctx.TerragruntOptions.Env["TERRAFUNK_SKIP_RUNCOMMANDS"] == "true" {
		return "SkippedCommandRuns", nil
	}

	//suppressOutput := false
	// Added For Terrafunk, Use Run Command Output To Determine Whether Output Is Suppressed
	// Defaults To Supressed
	suppressOutput := true
	if ctx.TerragruntOptions.Env["TERRAFUNK_RUN_CMD_OUT"] == "true" {
		suppressOutput = false
	}
...
func readTerragruntConfig(ctx *ParsingContext, configPath string, defaultVal *cty.Value) (cty.Value, error) {
	...
	if ctx.TerragruntOptions.WorkingDir == "." {
		path, err := os.Getwd()
		if err != nil {
			log.Println(err)
		}
		ctx.TerragruntOptions.WorkingDir = path
	}
